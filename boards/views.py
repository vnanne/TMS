        ...
        try:
            with transaction.atomic():
                ...
                Customer.objects.create(customer_name=data['contact_name'], customer_trade_name=data['company_name'],
                                        email=data['email'], telephone=data['ph_no'], tax_id=data['t_id'],
                                        motor_carrier=data['Carrier'])
            ...
            msg = str(' Customer details not added due to data issue  ') + str(e)
...
def Addterminal(request):
    data = request.POST
    if data['terminal_Id']:
        Terminals.objects.filter(id=data['terminal_Id']).update(terminal_name=data['terminal_name'],
                                                                email=data['email'], )
        Address.objects.filter(terminal_id=data['terminal_Id']).update(flat=data['street_add_1'],
                                                                       street=data['street_add_2'], city=data['City'],
                                                                       state=data['State'], country=data['Country'],
                                                                       zipcode=data['Zipcode'])
        ...
        try:
            ...
            msg = str(' Terminal details not added due to data issue  ') + str(e)
            messages.error(request, msg)
...
def Adddriver(request):
    ...
    if data['driver_Id']:
        Driver.objects.filter(id=data['driver_Id']).update(driver_name=data['display_name'], telephone=data['ph_no'],
                                                           email=data['email'],
                                                           ...
                                                           hazmat_endorsement=haz_switch,
                                                           dual_endorsement=dual_endors_switch,
                                                           tank_endorsement=tank_switch)
        Address.objects.filter(driver_id=data['driver_Id']).update(flat=data['street_add_1'],
                                                                   street=data['street_add_2'], city=data['City'],
                                                                   state=data['State'], country=data['Country'],
                                                                   zipcode=data['Zipcode'])
        ...
        try:
            with transaction.atomic():
                ...
                Driver.objects.create(driver_name=data['display_name'], telephone=data['ph_no'], email=data['email'],
                                      ...
                                      licence_issue_date=data['licence_issue_date'],
                                      licence_expiry_date=data['licence_expiry_date'], hazmat_endorsement=haz_switch,
                                      dual_endorsement=dual_endors_switch, tank_endorsement=tank_switch)
                Address.objects.create(flat=data['street_add_1'], street=data['street_add_2'], city=data['City'],
                                       state=data['State'], country=data['Country'], zipcode=data['Zipcode'],
                                       driver_id=Driver.objects.all().order_by("-id")[0].id)
    ...
    if data['I_load_type'] == 'import':
        ...
        try:
            ...
            with transaction.atomic():
                ...
                Workorder.objects.create(customer_id=customer_id, vessel_name=data['I_vessel_name'],
                                         pick_up_terminal_id=Terminals.objects.get(id=data['I_pickup_terminal']),
                                         return_terminal_id=Terminals.objects.get(id=data['I_return_terminal']))
                workorder_id = Workorder.objects.get(id=Workorder.objects.all().order_by('-id')[0].id)
                Import.objects.create(workorder_id=workorder_id, bill_of_landing_reference=data['I_bill_of_landing'],
                                      container=data['I_container_name'], contains_size=data['I_container_size'])
                Shipement_Details.objects.create(workorder_id=workorder_id, po=data['I_po_ref'],
                                                 weight=data['I_weight'], commidty=data['I_commodity'],
                                                 customs_hold=I_customs_hold_switch, over_weight=I_over_weight_switch)
                Appointments.objects.create(workorder_id=workorder_id, vessel_eat=data['ETA'], lfd=data['LFD'],
                                            port_apt=data['I_PortAPT'], delivert_apt=data['I_delivery_Appointment'])

                Address.objects.create(flat=data['I_delivery_Location'], street=data['I_street_add_1'],
                                       street_1=data['I_street_add_2'], city=data['I_city'], state=data['I_state'],
                                       country=data['I_country'], zipcode=data['I_zipcode'], workorder_id=workorder_id)
                Shipment_Specifications.objects.create(workorder_id=workorder_id, cost=data['I_cost'],
                                                       addition_cost=data['I_extra_costs'], remarks=data['I_remarks'],
                                                       tri_axl=I_tri_axle_switch, hazmat=I_hazmat_switch,
                                                       live=I_live_switch, drop=I_drop_pick_switch)
                Receiver.objects.create(contact_name=data['I_contact_Name'], email=data['I_email'],
                                        telephone=data['I_tel_no'], website=data['I_website'],
                                        delivery_notes=data['I_delivery_notes'], workorder_id=workorder_id)
        ...
        try:
            with transaction.atomic():
                ...
                Workorder.objects.create(customer_id=customer_id, vessel_name=data['vessel_name'],
                ...
                Shipement_Details.objects.create(workorder_id=workorder_id, po=data['po_ref'], weight=data['weight'],
                                                 ...
                                                 over_weight=over_weight_switch)
                Appointments.objects.create(workorder_id=workorder_id, vessel_eat=data['ETD'], lfd=data['Cut-OFF'],
                                            port_apt=data['PortAPT'], delivert_apt=data['pickup_Appointment'])

                Address.objects.create(flat=data['delivery_Location'], street=data['street_add_1'],
                                       street_1=data['street_add_2'], city=data['city'], state=data['state'],
                                       country=data['country'], zipcode=data['zipcode'], workorder_id=workorder_id)
                Shipment_Specifications.objects.create(workorder_id=workorder_id, cost=data['cost'],
            ...
            msg = str('NewLoad Export not successfully due to data issue please check the entered data  ') + str(e)
            messages.error(request, msg)
